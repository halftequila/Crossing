...
const CONFIG = {
    ...
    SUB_WORKER_URL: 'https://subhub.869069.xyz',
    SUBSCRIBER_URL: 'https://bestip-sub.869069.xyz/', //自定义订阅地址，可不用
    QUICK_SUB_URL: 'https://resub-name.869069.xyz/', //快速订阅地址，可不用
...
async function handleRequest(request, env) {
    ...
    // 返回管理界面
    return generateManagementPage(env);
}
...
function checkAuth(auth, env) {
    try {
        ...
        const validUsername = env.ADMIN_USERNAME || CONFIG.DEFAULT_USERNAME;
        const validPassword = env.ADMIN_PASSWORD || CONFIG.DEFAULT_PASSWORD;
...
// 修改生成管理页面函数
function generateManagementPage(env) {
    const subWorkerUrl = formatUrl(env.SUB_WORKER_URL || CONFIG.SUB_WORKER_URL);
    const templateUrl = env.DEFAULT_TEMPLATE_URL || CONFIG.DEFAULT_TEMPLATE_URL;
    const username = env.ADMIN_USERNAME || CONFIG.DEFAULT_USERNAME;
    const password = env.ADMIN_PASSWORD || CONFIG.DEFAULT_PASSWORD;
    ...
    return new Response(
        ...
        '<head>' +
            ...
            '<meta name="sub-worker-url" content="' + subWorkerUrl + '">' +
            '<meta name="template-url" content="' + templateUrl + '">' +
            '<meta name="auth" content="Basic ' + btoa(username + ':' + password) + '">' +
            '<meta name="singbox-url" content="' + CONFIG.SINGBOX_URL + '">' +
            '<meta name="clash-url" content="' + CONFIG.CLASH_URL + '">' +
            '<style>' +
            // 基础样式和布局
            ...
            'h3 { margin: 0 0 10px 0; color: #444; font-size: 16px; }' +
            ...
            // 节点和集合样式
            '.node-item, .collection-item { background: #f8f9fa; padding: 12px; border-radius: 8px; margin-bottom: 12px; }' +
            '.node-content { margin: 8px 0; word-break: break-all; font-size: 14px; color: #666; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%; }' +
            ...
            '.node-selection { ' +
                ...
                'display: grid; ' +
                'grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)); ' +  // 减小最小宽度
                'gap: 8px; ' +  // 减小间距
            '}' +
        ...
        '</div>' +
                ...
                '<div class="input-group">' +
                    ...
                    '<button onclick="openSubscriber()" class="button" style="background-color: #ff6b6b;">自选订阅器</button>' +
                    '<button onclick="openQuickSubscriber()" class="button" style="background-color: #ff6b6b;">快速订阅器</button>' +  // 添加新按钮
                '</div>' +
            ...
            '<script>' +
                ...
                'const SINGBOX_URL = "' + CONFIG.SINGBOX_URL + '";' +
                'const CLASH_URL = "' + CONFIG.CLASH_URL + '";' +
                    ...
                    'collectionList.innerHTML = collections.map(collection => {' +
                            ...
                            'return `' +
                                ...
                                '<div class="button-group">' +  // 添加按钮组
                                    ...
                                    '<button onclick="singboxSubscription(\'${collection.id}\')" class="button" style="background-color: #4a9eff;">SingBox订阅</button>' +  // 新增这一行
                                    '<button onclick="clashSubscription(\'${collection.id}\')" class="button" style="background-color: #4a9eff;">Clash订阅</button>' +  // 添加这一行
        ...
        'async function universalSubscription(id) {' +
            'const shareUrl = window.location.origin + "/api/share/" + id;' +
            'const subWorkerUrl = document.querySelector("meta[name=\\"sub-worker-url\\"]").content;' +
            'const subUrl = `${subWorkerUrl}/base?url=${encodeURIComponent(shareUrl)}`;' +
            'try {' +
        ...
        'async function singboxSubscription(id) {' +
            'const shareUrl = window.location.origin + "/api/share/" + id;' +
            'const subWorkerUrl = document.querySelector("meta[name=\\"sub-worker-url\\"]").content;' +
            'const templateUrl = document.querySelector("meta[name=\\"template-url\\"]").content;' +
            'const subUrl = `${subWorkerUrl}/singbox?url=${encodeURIComponent(shareUrl)}&template=${encodeURIComponent(templateUrl)}`;' +
            'try {' +
                'await navigator.clipboard.writeText(subUrl);' +
                'alert("SingBox订阅链接已复制到剪贴板");' +
            '} catch (e) {' +
                'alert("复制SingBox订阅链接失败");' +
            '}' +
        '}' +

        'async function clashSubscription(id) {' +
            'const shareUrl = window.location.origin + "/api/share/" + id;' +
            'const subWorkerUrl = document.querySelector("meta[name=\\"sub-worker-url\\"]").content;' +
            'const templateUrl = document.querySelector("meta[name=\\"template-url\\"]").content;' +
            'const subUrl = `${subWorkerUrl}/clash?url=${encodeURIComponent(shareUrl)}&template=${encodeURIComponent(templateUrl)}`;' +
            'try {' +
                'await navigator.clipboard.writeText(subUrl);' +
                'alert("Clash订阅链接已复制到剪贴板");' +
            '} catch (e) {' +
                'alert("复制Clash订阅链接失败");' +
            '}' +
        '}' +
    ...
    '}' +
        ...
        'function openQuickSubscriber() {' +
            'const quickSubUrl = "' + CONFIG.QUICK_SUB_URL + '";' +
            'if (quickSubUrl) {' +
